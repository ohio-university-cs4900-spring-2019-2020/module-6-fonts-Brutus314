...
#include <vector>
#include <memory>
#include "GLViewFonts.h" //GLView subclass instantiated to drive this simulation
...
int main(int argc, char* argv[])
{
	...
	int simStatus = 0;
	if (argc != 2) std::cerr << "Need 1 input argument to be your username!";
	else {
		do
		{
			std::unique_ptr< Aftr::GLViewFonts > glView(Aftr::GLViewFonts::New(args));
